...
(define-map affiliate-records 
  { affiliate: principal, period: uint } ;; Keys: affiliate principal and period ID
  {
...
(define-map period-rankings
  { period-id: uint }
  { 
    rankings: (list 100 { affiliate: principal, score: uint }),  ;; Top affiliates with their scores
...
(define-map reward-claims
  { affiliate: principal, period-id: uint }
  { 
...
(define-private (calculate-affiliate-score 
  ...
  (let
    ...
    (/ (+ (+ sales-score commission-score) conversion-score) u10)  ;; Normalize to get final score
  )
)
...
(define-public (record-sale 
  ...
  (let
    ...
    (match (map-get? affiliate-records { affiliate: affiliate, period: period-id })
      existing-record
        (let
          ...
          (map-set affiliate-records
            { affiliate: affiliate, period: period-id }
            {
...
(define-public (start-new-period (period-type (string-utf8 10)) (duration uint))
  (let
    ((current-time (get-current-time))
    ...
    (asserts! (or (is-eq period-type "weekly") (is-eq period-type "monthly")) ERR-INVALID-PERIOD)
    ...
    (match (map-get? periods { period-id: period-id })
      period
        (begin
          ...
          (asserts! (>= current-time (get end-time period)) ERR-PERIOD-NOT-ENDED)
          
          ;; Check that period is not already finalized
          (asserts! (not (get is-finalized period)) (err u108))
          
          ;; Here we would ideally gather all affiliate data and sort by score
          ;; Since Clarity doesn't have looping constructs, in a real-world implementation
          ;; this would likely be done through an off-chain process and the results
          ;; would be submitted by the contract owner.
          
          ;; Mark period as finalized
          (map-set periods
            { period-id: period-id }
            {
              start-time: (get start-time period),
              end-time: (get end-time period),
              is-finalized: true,
              period-type: (get period-type period)
            }
          )
          
          ;; For testing purposes, we'll create an empty rankings list
          (map-set period-rankings
            { period-id: period-id }
            {
              rankings: (list 100 { affiliate: tx-sender, score: u0 }),
...
(define-public (distribute-rewards (period-id uint) (affiliate principal) (reward-amount uint))
  (let
    ((current-time (get-current-time)))
    ...
    ;; Check reward hasn't been claimed yet
    (match (map-get? reward-claims { affiliate: affiliate, period-id: period-id })
      existing-claim
        (asserts! (not (get claimed existing-claim)) ERR-ALREADY-CLAIMED)
    ...
    (map-set reward-claims
      { affiliate: affiliate, period-id: period-id }
      {
        claimed: true,
...
(define-read-only (get-affiliate-metrics (affiliate principal) (period-id uint))
  (match (map-get? affiliate-records { affiliate: affiliate, period: period-id })
    record (ok record)
...
(define-read-only (get-period-top-performers (period-id uint) (limit uint))
  (match (map-get? period-rankings { period-id: period-id })
    rankings 
      (ok (get rankings rankings))
    (err ERR-INVALID-PERIOD)
  )
)

;; Check if an affiliate has claimed rewards for a period
(define-read-only (has-claimed-reward (affiliate principal) (period-id uint))
  (match (map-get? reward-claims { affiliate: affiliate, period-id: period-id })
...
(define-read-only (calculate-current-score (affiliate principal))
  (let
    ...
    (match (map-get? affiliate-records { affiliate: affiliate, period: period-id })
      record 
        (ok (calculate-affiliate-score 
              (get sales-count record) 
              (get total-sales-amount record) 
              (get commission-earned record) 
