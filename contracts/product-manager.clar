...
(define-map product-categories
  { product-id: uint, category-id: uint }
  { assigned-at: uint }
)
...
(define-map bundle-products
  { bundle-id: uint, product-id: uint }
  { added-at: uint }
)
...
(define-read-only (is-product-in-category (product-id uint) (category-id uint))
  (is-some (map-get? product-categories { product-id: product-id, category-id: category-id }))
)

;; Check if product is in bundle
(define-read-only (is-product-in-bundle (product-id uint) (bundle-id uint))
  (is-some (map-get? bundle-products { bundle-id: bundle-id, product-id: product-id }))
)
...
(define-read-only (get-active-discount (product-id uint))
  (let ((discount-data (map-get? product-discounts { product-id: product-id })))
    (if (is-some discount-data)
      (let ((discount (unwrap-panic discount-data))
            (current-time (get-current-time)))
        (if (and (>= current-time (get start-time discount))
...
(define-public (set-product-featured (product-id uint) (featured bool))
  (let ((product-data (map-get? products { product-id: product-id })))
    (if (is-none product-data)
      ERR-PRODUCT-NOT-FOUND
      (if (not (is-product-merchant product-id))
...
(define-public (create-category (name (string-ascii 50)) 
  ...
  (let ((category-id (var-get next-category-id))
    ...
    (if (and (is-some parent-category-id) 
...
(define-public (add-product-to-category (product-id uint) (category-id uint))
  (let ((product-data (map-get? products { product-id: product-id }))
        ...
        (if (not (is-product-merchant product-id))
          ...
          (begin
            (map-set product-categories
              { product-id: product-id, category-id: category-id }
...
(define-public (remove-product-from-category (product-id uint) (category-id uint))
  (let ((product-data (map-get? products { product-id: product-id }))
        (category-data (map-get? categories { category-id: category-id })))
    (if (is-none product-data)
      ERR-PRODUCT-NOT-FOUND
      (if (is-none category-data)
        ...
        (if (not (is-product-merchant product-id))
          ERR-NOT-AUTHORIZED
          (begin
            (map-delete product-categories { product-id: product-id, category-id: category-id })
            (ok true)
          )
        )
      )
...
(define-public (set-product-discount (product-id uint) 
                                    (discount-percentage uint) 
                                    (start-time uint) 
                                    (end-time uint))
  (let ((product-data (map-get? products { product-id: product-id }))
...
(define-public (create-bundle (name (string-ascii 100)) 
                             (description (string-utf8 300)) 
                             (price uint))
...
;; Update bundle
(define-public (update-bundle (bundle-id uint) 
                             (name (string-ascii 100)) 
                             (description (string-utf8 300)) 
                             (price uint)
                             (active bool))
...
(define-public (add-product-to-bundle (bundle-id uint) (product-id uint))
  (let ((bundle-data (map-get? bundles { bundle-id: bundle-id }))
          ...
          (if (is-product-in-bundle product-id bundle-id)
            ...
            (begin
              (map-set bundle-products
                { bundle-id: bundle-id, product-id: product-id }
...
(define-public (remove-product-from-bundle (bundle-id uint) (product-id uint))
  (let ((bundle-data (map-get? bundles { bundle-id: bundle-id })))
    (if (is-none bundle-data)
      ERR-BUNDLE-NOT-FOUND
      (if (not (is-bundle-merchant bundle-id))
        ...
        (if (not (is-product-in-bundle product-id bundle-id))
          ...
          (begin
            (map-delete bundle-products { bundle-id: bundle-id, product-id: product-id })
            (ok true)
          )
        )
      )
...
;; Record product sale
(define-public (record-product-sale (product-id uint) (sale-amount uint) (is-affiliate-sale bool))
  (let ((metrics-data (map-get? product-metrics { product-id: product-id }))
        (product-data (map-get? products { product-id: product-id })))
    (if (or (is-none metrics-data) (is-none product-data))
      ERR-PRODUCT-NOT-FOUND
      (let ((current-metrics (unwrap-panic metrics-data))
            (product (unwrap-panic product-data)))
        ;; Only allow the merchant to record sales
        (if (not (is-eq tx-sender (get merchant product)))
          ERR-NOT-AUTHORIZED
          (map-set product-metrics
            { product-id: product-id }
            {
              view-count: (get view-count current-metrics),
              affiliate-sales-count: (if is-affiliate-sale
                                      (+ (get affiliate-sales-count current-metrics) u1)
                                      (get affiliate-sales-count current-metrics)),
              direct-sales-count: (if (not is-affiliate-sale)
                                    (+ (get direct-sales-count current-metrics) u1)
                                    (get direct-sales-count current-metrics)),
              revenue: (+ (get revenue current-metrics) sale-amount),
              last-updated: (get-current-time)
            }
          )
        )
