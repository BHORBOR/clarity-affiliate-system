...
(define-map commission-splits
  ...
  { 
    recipients: (list 10 { recipient: principal, share: uint }),  ;; Share is in basis points (total must be 10000)
    active: bool
  }
)
...
(define-private (is-valid-commission-split (recipients (list 10 { recipient: principal, share: uint })))
  (is-eq (fold + (map get-share recipients) u0) u10000)
)

;; Helper to get share from recipient object
(define-private (get-share (recipient-info { recipient: principal, share: uint }))
  (get share recipient-info)
)

;; Add pending payout amount to recipient
(define-private (add-to-pending-payout (recipient principal) (amount uint))
  (let ((current-pending (default-to { amount: u0, last-updated: (get-block-height) }
...
(define-private (process-payout (recipient principal) (amount uint))
...
(define-private (record-payout 
                 ...
                 (recipients (list 10 { recipient: principal, amount: uint }))
                 (total-amount uint))
  (let ((tx-id (unwrap-panic (get-txid))))
    ...
    (lambda (split) 
      {
        ...
        amount: (/ (* total-amount (get share split)) u10000)
      }) 
    splits)
)
...
(define-read-only (get-commission-split (merchant principal) (affiliate principal))
  (map-get? commission-splits { merchant: merchant, affiliate: affiliate })
)
...
(define-read-only (get-payout-detail (tx-id (buff 32)))
...
(define-read-only (is-payout-time (schedule uint))
  (let ((current-height (get-block-height)))
    (cond
      ((is-eq schedule SCHEDULE-IMMEDIATE) true)
      ((is-eq schedule SCHEDULE-DAILY) (>= current-height (var-get next-daily-payout)))
      ((is-eq schedule SCHEDULE-WEEKLY) (>= current-height (var-get next-weekly-payout)))
      ((is-eq schedule SCHEDULE-MONTHLY) (>= current-height (var-get next-monthly-payout)))
      (true false)
    )
  )
)
...
(define-public (set-administrator (admin principal) (active bool))
  (begin
...
(define-public (set-merchant-config 
  ...
  (begin
    ...
    (asserts! (and (>= payout-schedule SCHEDULE-IMMEDIATE) (<= payout-schedule SCHEDULE-MONTHLY)) ERR-INVALID-PAYOUT-SCHEDULE)
    (asserts! (<= default-commission-rate u10000) ERR-INVALID-COMMISSION-SPLIT)
    
    (ok (map-set merchant-config
                { merchant: merchant }
                {
...
(define-public (set-commission-split 
  ...
  (begin
    ...
    (ok (map-set commission-splits
                { merchant: merchant, affiliate: affiliate }
                {
                  recipients: recipients,
                  active: active
                }))
...
(define-public (record-commission 
                ...
                (affiliate principal) 
                (amount uint))
  (let ((merchant-cfg (unwrap! (get-merchant-config merchant) ERR-MERCHANT-NOT-FOUND))
        (commission-split (map-get? commission-splits { merchant: merchant, affiliate: affiliate })))
    ...
    (if (and (is-some commission-split) (get active (default-to { active: false } commission-split)))
        ...
        (let ((recipients (get recipients (unwrap-panic commission-split)))
              (recipient-amounts (calculate-recipient-amounts amount recipients)))
          
          ;; First record payouts to all recipients
          (map add-to-pending-payout 
               (map get-recipient recipient-amounts)
               (map get-amount recipient-amounts))
...
(define-private (get-recipient (item { recipient: principal, amount: uint }))
  (get recipient item)
)

(define-private (get-amount (item { recipient: principal, amount: uint }))
  (get amount item)
)
...
(define-public (process-recipient-payout (recipient principal))
  (let ((pending (unwrap! (get-pending-payout recipient) ERR-NO-PENDING-PAYOUTS))
...
(define-public (update-payout-threshold (merchant principal) (new-threshold uint))
